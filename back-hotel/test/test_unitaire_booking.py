...
class TestbookingPut(unittest.TestCase):
    def setUp(self):
        app.testing = True
        self.app_context = app.app_context()
        self.app_context.push()
    ...
    def test_update_booking_as_admin(self):
        with self.client:
            user_user = user.query.filter_by(email="user@example.com").first()
            booking_obj = booking(chambre_id=self.chambre1.id, user_id=user_user.id, datein='2024-03-01', dateout='2024-03-05')
            db.session.add(booking_obj)
            db.session.commit()

            data = {'datein': '2024-03-02'}
            response = self.client.put(f'/booking/{booking_obj.id}', json=data, headers={'Authorization': f'Bearer {self.admin_token}'})
            
            updated_booking = booking.query.get(booking_obj.id)
    ...
    def test_update_booking_as_user(self):
        with self.client:
            ...
            data = {'dateout': '2024-03-06'}
            response = self.client.put(f'/booking/{booking_obj.id}', json=data, headers={'Authorization': f'Bearer {self.user_token}'})
            updated_booking = booking.query.get(booking_obj.id)
            db.session.delete(updated_booking)
            db.session.commit()
    ...
    def test_update_booking_missing_parameters(self):
        with self.client:
            user_user = user.query.filter_by(email="user@example.com").first()
            booking_obj = booking(chambre_id=self.chambre1.id, user_id=user_user.id, datein='2024-03-01', dateout='2024-03-05')
            db.session.add(booking_obj)
            ...
            response = self.client.put(f'/booking/{booking_obj.id}', json={}, headers={'Authorization': f'Bearer {self.user_token}'})
            data = response.get_json()
    ...
    def test_update_booking_unauthorized(self):
        with self.client:
            admin_user = user.query.filter_by(email="admin@example.com").first()
            booking_obj = booking(chambre_id=self.chambre1.id, user_id=admin_user.id, datein='2024-03-01', dateout='2024-03-05')
            db.session.add(booking_obj)
            db.session.commit()
            ...
            self.assertIn('error', data)
            self.assertEqual(data['error'], 'You have not the permission for modify this')
...
class TestbookingDelete(unittest.TestCase):
    def setUp(self):
        app.testing = True
        self.app_context = app.app_context()
        self.app_context.push()
    ...
    def test_delete_booking_as_admin(self):
        with self.client:
            ...
            response = self.client.delete(f'/booking/{booking_obj.id}', headers={'Authorization': f'Bearer {self.admin_token}'})
            deleted_booking = booking.query.get(booking_obj.id)
    ...
    def test_delete_booking_as_user(self):
        with self.client:
            ...
            response = self.client.delete(f'/booking/{booking_obj.id}', headers={'Authorization': f'Bearer {self.user_token}'})
            deleted_booking = booking.query.get(booking_obj.id)
    ...
    def test_delete_booking_unauthorized(self):
        with self.client:
            user_user = user.query.filter_by(email="admin@example.com").first()
            booking_obj = booking(chambre_id=self.chambre1.id, user_id=user_user.id, datein='2024-03-01', dateout='2024-03-05')
            db.session.add(booking_obj)
            db.session.commit()
    ...
    def test_delete_non_existing_booking(self):
        with self.client:
            response = self.client.delete('/booking/999', headers={'Authorization': f'Bearer {self.admin_token}'})
            data = response.get_json()

            self.assertEqual(response.status_code, 404)
            self.assertIn('error', data)
            self.assertEqual(data['error'], 'booking not found')
